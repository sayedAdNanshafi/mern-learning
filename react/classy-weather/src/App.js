...
class App extends React.Component{
  constructor(props){
    super(props)
    this.state={location:'',isLoading:false,displayLocation:'',weather:{}}
  ...
  render(){
    ...
    return <div>
      ...
      <ul className="weather">
        {dates.map((date,i)=><Day date={date} max={max.at(i)} min={min.at(i)} code={codes.at(i)} key={date} isToday={i===0} isTomorrow={i===1}/>)}
      </ul>
    </div>
  }
}
class Day extends React.Component{
  render(){
    const {date,min,max,code,isToday,isTomorrow}=this.props
    return <li className="day">
      <span>{getWeatherIcon(code)}</span>
      <p>{isToday?'Today':isTomorrow?'Tomorrow':formatDay(date)}</p>
